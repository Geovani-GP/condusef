...
/**@preserve GeneXus 10.3.15.115793*/
					...
					popupMinWidth: function () {
						...
						if (currentPopup) {
							if (gx.runtimeTemplates) {
								...
								$(document.documentElement).css("width", gx.dom.addUnits(minWidth - this.scrollbarWidth()));
				...
				loadScript: function (url, callback) {
					...
					if (typeof (callback) == 'function') {
						if (script.addEventListener)
							gx.evt.attach(script, ['load', 'error'] ,callback, { userCapture: false} );
						else {
			...
			return {
				...
				radio: function (id, width, height, cssClass, title) {
					...
					this._getHtml = function () {
						...
						var templateName = (this.orientation == 1) ? "radio-vertical" : "radio-horizontal";
						this.append(applyTemplate(templateName, {
							id: this.id,
							className: ClassHTML,
							style: this.style,
							title: this.title,
	...
	return {
		...
		ext: {
			...
			window: function (popupObj, autoSize, x, y, w, h, contentHtml, bgcolor, title, titlecolor, bordercolor, scrollcolor, shadowcolor, ismodal, showonstart, isdrag, isresize, isExt, rsImg, headCls, btnCls, cntCls, brdCls, shdwCls, showParentPopups, swallowKeys, callbacks, parentElement) {
				...
				outerdiv.className = gx.runtimeTemplates? "gx-responsive-popup " + brdCls: brdCls;
			...
			popUp: function (popupObj, autoSize, popCentered, x, y, w, h, cid, text, bgcolor, textcolor, fontstyleset, title, titlecolor, titletextcolor, bordercolor, scrollcolor, shadowcolor, ismodal, showonstart, isdrag, isresize, oldOK, isExt, popOnce, minImg, maxImg, clsImg, rsImg, headCls, btnCls, cntCls, brdCls, shdwCls, showParentPopups) {
				...
				if (!popupObj.IsPrompt && !gx.util.sameAppUrl(text)) {
					...
					var intialHeight = (gx.HTML5)? 'auto': '100%';
					var intitialWidth = (gx.runtimeTemplates)? gx.dom.fixes.getPopupMinWidth() + gx.dom.fixes.scrollbarWidth(): intialHeight;
					var contentHtml = '<iframe id="' + cid + '_ifrm" src="' + text + '" width="' + intitialWidth + '" height="' + intialHeight + '" frameborder="0" style="overflow:' + overflow + ';" title="' + iframeTitle + '"></iframe>';
			...
			autofit: function (popupObj, frameDoc, doShow) {
				...
				if (popupObj.autoresize) { //autresize != 0
					if (!isPDFPopup && frameDoc) {
						...
						var scrollWidth, 
							scrollbarwidth = (gx.util.browser.isEdge())? gx.dom.fixes.scrollbarWidth(): 0;
						if (mainEl) {
				...
				getInputParms: function () {
					...
					for (var i = 0, len = parmsMetadata.length; i < len; i++) {
						...
						if (parm.postForm)	{
							...
							if (parm.prop) {
								value = this.gxO.getCtrlPropertyValue(parm.ctrl, parm.prop);
							} 
							else {
		...
		dispatchCommands: function (Commands, gxO, ignoreCmds) {
			...
			if (Commands && Commands.length > 0) {
				...
				for (var i = 0; i < len; i++) {
					...
					if (Command.refresh !== undefined || Command.refresh_form !== undefined) {
...
gx.grid.impl = (function ($) {
	return function (id) {
		...
		this.appendCollapsingWrapperStart = function (buffer) {
			buffer.append("<table><tr data-cell-valign=\"top\"><td>");
		...
		this.isGridRow = function (el) {
			return el && el.tagName == 'TR' && el.getAttribute("data-gxrow") && gx.dom.findParentByTagName(el, 'DIV') == this.container;
		};
...
gx.GxObject = (function($) {
	var gxObject = function () {
		...
		this.createWebComponent = function (CtrlName, PgmName, Parms, gRow, TargetCtrlId, CmpPrefix) {
			...
			gx.ajax.dynComponent(javaPackage + PgmName, Parms, CmpPrefix, gRow).done(function (srvResponse) {
				...
				gx.ajax.setPostResponse(srvResponse, gxO);
				if (gxO) {
					gxO.endFeedback();
				}
				else {
	...
	gxObject.CONTROL_CMP_REGEX = /((?:(?:MP)?W[0-9]{4})*)((?:MP)?W[0-9]{4})/;
	...
	return {
		...
		loadRadioButton: function (control, values, selectedValue) {
			var $parent = $(control).parent().closest('.gx-radio-button'),
				templateName = $parent.is("table") ? "radio-vertical" : "radio-horizontal",
				$control = $(control),
				$newControl;

			$newControl = $(gx.html.controls.applyTemplate(templateName, {
				id: control.name,
		...
		getCtrlProperty: function (ControlId, Property) {
			var Control;
			Control = gx.uc.getUserControlObj(gx.uc.userControlContainerId(ControlId));
			if (Control != null)
		...
		doAttachs: function (newComponents) {
			...
			var gxO = gx.O;
			var newCmpContexts = newComponents && newComponents.length ? $.map(newComponents, gx.fn.cmpContextFromCtrl) : undefined;
			for (var j = 0; j < len; j++) {
...
gx.util.alert = (function ($) {
	...
	return {			
		...
		showError: function(message) {
			...
			gxError.html('<div class="alert alert-danger alert-dismissable gx-ajax-error"><button type="button" class="close" data-dismiss="alert" onclick=\'$(this).parent().hide();\' aria-hidden="true">&times;</button><span>'+message+'</span></div>');
			gxError.slideDown();
			shown = true;
		},
